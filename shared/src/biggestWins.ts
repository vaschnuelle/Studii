 ...
 */
export function computeBiggestWins(
  ...
  const pomodorosByDay = new Map<string, number>();
 ...
 */
function computeMostConsistentWeek(minutesByDay: Map<string, number>, nowMs: number): string {
  ...
  for (let w = 0; w <= series.length - 7; w++) {
    ...
    const variance =
      chunk.reduce((acc, x) => acc + (x - mean) * (x - mean), 0) / 7;
  ...
  if (!Number.isFinite(bestStd)) return "—";
 ...
 */
function computeMonthImprovementPercent(
  minutesByDay: Map<string, number>,
  nowMs: number
): number | null {
