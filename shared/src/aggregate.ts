import { computeBiggestWins } from "./biggestWins";
import {
  buildLast30dDistributions,
  medianSessionMinutes,
  powerHourAndDay,
  totalHoursLast30d,
} from "./distributions";
import { buildFriendOverlapCache } from "./overlap";
...
import type {
  ...
  SessionRecord,
  UserStats,
  WrappedSummaryRecord,
} from "./types";

/**
 * Derives personality inputs from sessions and overlap cache (deterministic).
 */
export function buildPersonalityInputs(
  sessions: SessionRecord[],
  friendOverlapCache: Record<string, number>,
  nowMs: number = Date.now()
): PersonalityInputs {
  const { hour } = buildLast30dDistributions(sessions, nowMs);
  let peakIdx = 0;
  for (let i = 1; i < 24; i++) {
    if (hour[i]! > hour[peakIdx]!) peakIdx = i;
  }
  ...
  for (const s of sessions) {
    if (s.isPomodoro) {
      pomodoroSessions += 1;
      if ((s.pomodoroCycles ?? 0) > 0) pomodoroCompleted += 1;
    }
  }
  const pomodoroCompletionRate =
    pomodoroSessions > 0 ? pomodoroCompleted / pomodoroSessions : 0.5;

  return {
    medianSessionMin: medianSessionMinutes(sessions, nowMs),
    peakHour: peakIdx,
...
export function buildUserStats(
  ...
  const { hour, weekday } = buildLast30dDistributions(mySessions, nowMs);
  const friendOverlapCache = buildFriendOverlapCache(mySessions, friendSessionsById, nowMs);
  const inputs = buildPersonalityInputs(mySessions, friendOverlapCache, nowMs);
  const personality: PersonalityArchetype = classifyPersonality(inputs);
 ...
 */
export function weekPeriodId(d: Date = new Date()): string {
...
export function buildWrappedSummary(
  mySessions: SessionRecord[],
  friendSessionsById: Record<string, SessionRecord[]>,
  periodId: string = weekPeriodId(),
  nowMs: number = Date.now()
): WrappedSummaryRecord {
  const stats = buildUserStats(mySessions, friendSessionsById, nowMs);
  const { hour, weekday } = buildLast30dDistributions(mySessions, nowMs);
  const { powerHour, powerDay } = powerHourAndDay(hour, weekday);
  const wins = computeBiggestWins(mySessions, nowMs);
  ...
  return {
    ...
    totalHours: Math.round(totalHoursLast30d(mySessions, nowMs) * 1e2) / 1e2,
    shareCardUrl: null,
